...
# — количество значений x, для которых требуется узнать значение функции f(x), после чего сами эти n значений, каждое на отдельной строке. Программа должна после каждого 
# введённого значения аргумента вывести соответствующие значения функции f
# на отдельной строке.
...
for i in range(0,n):
    x = int(input())
# Проверка наличия ранее вычисленного значения функции f(x) для данного значения x
    if x in d:
        print(d[i])
    else:
# Добавление в словарь значение функции f(x) для значения x
        d[x] = f(x)
        print(f(x))